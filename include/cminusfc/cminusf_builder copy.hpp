...
class CminusfBuilder : public ASTVisitor {
  ...
  private:
    ...
    struct {
        ...
        // Current block for IR generation
        BasicBlock *bb = nullptr;

        // Maintains break and continue block info for loops
        BasicBlock *break_block = nullptr;
        BasicBlock *continue_block = nullptr;

        // Handle return values if needed for return processing
        Value *return_value = nullptr;

        bool false_exit=false;

        int judge_count=0;

        int select_count=0;

        bool if_in_main=false;
    } context;
};
